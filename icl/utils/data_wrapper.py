...
def tokenize_dataset(dataset, tokenizer):
    def tokenize_function(examples):
        return tokenizer(examples["sentence"], padding=True,
                         ...
                         return_tensors='pt')
    '''
    datasets中的Dataset实例看做是一张数据表。
    map方法会将输入的function按照指定的方式应用在每一行（每一行称为一个example）上
    当设置batched=False时，可调用对象依次处理每一行
    当设置batched=True时，可调用对象会对样本进行批处理，批的大小可以通过batch_size控制，默认一个批为1000条样本
    '''
